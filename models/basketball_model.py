"""
Created on Mon Nov  3 22:26:38 2015
@author: Max W. Y. Lam
"""
import sys
sys.path.append("../max-ssgp-python")
import os
import csv
import numpy as np
...
class basketball_model:
    ...
    def __init__(self, database_path="NBA_database/", model_path="trained_GPR/"):
    ...
    def eval_accuracy_by_date(self, year, month, day, test_nums=[20, 50, 100]):
        ...
        for i in xrange(len(self.game_dict["date"])):
        ...
        for i in test_ind[max(test_nums)].keys():
            ...
            print("\nIf", team1, "confronts", team2, "on", year, month, day, "then...")
            ...
            print("In fact,", true_win_team, "win!")
            if(pred_win_team == "---"):
                ...
                if(pred_win_team == true_win_team):
                    print("The predictor is correct!")
                    for j, test_num in enumerate(test_nums):
                        if(i in test_ind[test_num].keys()):
                            count_correct[j] += 1
                else:
                    print("The predictor is incorrect!")
                    for j, test_num in enumerate(test_nums):
                        if(i in test_ind[test_num].keys()):
                            count_incorrect[j] += 1
    ...
    def eval_accuracy(self, eval_season=None, plot_correct_dist=False, threshold=0.5):
        ...
        if(plot_correct_dist):
            ...
            plt.figure()
            plt.hist(self.correct_distribution, bins, range=[0.5, 1], alpha=0.5, label='Corr Freq')
            plt.hist(self.incorrect_distribution, bins, range=[0.5, 1], alpha=0.5, label='Incorr Freq')
                ...
                for G_model in G_models:
                    ...
                    for WIN_model in WIN_models:
                        ...
                        if(_accuracy > accuracy):
                            ...
                            best_models = models
        import shutil
        shutil.copy2(best_models[0], self.get_player_model_path_by_pos("C"))
    ...
    def predict_match(self, team1, team2, year, month, day, threshold=0.5):
    ...
    def load_winning_team_model(self):
        print("loading trained SSGP of Winning Team Predict Model ......")
        self.loaded_winning_team_model = SSGP()
        self.loaded_winning_team_model.load(self.get_winning_team_model_path())
        print("loaded SSGP", self.loaded_winning_team_model.hashed_name)

    def load_player_models(self):
        print("loading trained SGPR of +/- ......")
    ...
    def get_winning_team_model_path(self, name=""):
        file_dir = os.path.dirname(os.path.realpath('__file__'))
        if(name == ""):
    ...
    def get_player_models_dataset(self):
        ...
        for pos in self.all_position:
            ...
            for player in self.position_to_player[pos]:
                ...
                for date_id in xrange(len(plus_minus_vec)-1):
    ...
    def train_player_models(self):
        ...
        for i in rand_permu:
            ...
            if("-" in pos):
                pos = pos[0]
            file_dir = os.path.dirname(os.path.realpath('__file__'))
            model_dir = os.path.join(file_dir, "models/basketball_models/")
            n = dataset[1].shape[0]
            X = dataset[0].copy()
            y = dataset[1].copy()
            ...
            if(len(models) == 1):
                ...
                min_nmse, min_mnlp = best_model.predict(X.copy(), y.copy())
    ...
    def get_winning_team_model_dataset(self):
    ...
    def train_winning_team_model(self):
        ...
        if(self.winning_team_model_dataset is None):
            self.get_winning_team_model_dataset()
