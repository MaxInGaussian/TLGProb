"""
Created on Sun Oct  28 21:42:28 2015
@author: Max W. Y. Lam
"""
import os
import csv
...
class basketball_bot(bot):
    ...
    def get_all_games_by_year(self, year, update=True):
        ...
        rows = self._new_command(br_id, (self.FINDI, "games",
                                         self.FINDT, "tr"))[1]
        for row in rows:
            cols = self._find_elements(row, self.FINDT, "td")
            if(len(cols) < 9):
                continue
            for key, i in key_to_i.items():
                ...
                if(key == "team1pts" or key == "team2pts"):
                    ...
                    name_a = self._find_elements(cols[i], self.FINDT, "a")
                    if(len(name_a) != 1):
                        print(s + "Error!")
    ...
    def get_basketball_game_log_by_url(self, game_log_url, log, update):
        ...
        rows = self._new_command(br_id, (self.FINDI, "pgl_basic",
                                         self.FINDT, "tr"))[1]
        for row in rows:
            cols = self._find_elements(row, self.FINDT, "td")
            if(len(cols) < 30):
    ...
    def get_basketball_player_by_name_char(self, c, update=True):
        ...
        br_id = self._get_any_idle_br_id()
        self._new_command(br_id, (self.GOTO, url))
        rows = self._new_command(br_id, (self.FINDI, "players",
                                         self.FINDT, "tr"))[1]
        for row in rows:
            cols = self._find_elements(row, self.FINDT, "td")
            if(len(cols) != 8):
                continue
            name_a = self._find_elements(cols[0], self.FINDT, "a")
            if(len(name_a) != 1):
                continue
            link = str(name_a[0].get_attribute("href"))
            if(url not in link):
